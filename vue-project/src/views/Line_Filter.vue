...
async function Get_Champion() {
    ...
    const res = await fetch(
        `https://ddragon.leagueoflegends.com/cdn/${Version_Store.version}/data/ko_KR/champion.json`
    );
...
async function Get_line_Champion() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        Line_Champions.value[line[i]] = data.sort((a, b) =>
...
<template>
    <div class="filter-container" v-if="State_Store.state.phase != 'Done'">
        <div class="Position-Filters">
            <img
                src="/src/assets/data/top_icon.png"
                class="filter"
                :class="{ filtered: is_filtered('top') }"
                @click="champ_filter('top')"
            />
            <img
                src="/src/assets/data/jug_icon.png"
                class="filter"
                :class="{ filtered: is_filtered('jug') }"
                @click="champ_filter('jug')"
            />
            <img
                src="/src/assets/data/mid_icon.png"
                class="filter"
                :class="{ filtered: is_filtered('mid') }"
                @click="champ_filter('mid')"
            />
            <img
                src="/src/assets/data/bot_icon.png"
                class="filter"
                :class="{ filtered: is_filtered('bot') }"
                @click="champ_filter('bot')"
            />
            <img
                src="/src/assets/data/sup_icon.png"
                class="filter"
                :class="{ filtered: is_filtered('sup') }"
                @click="champ_filter('sup')"
            />
        </div>
    </div>
</template>
