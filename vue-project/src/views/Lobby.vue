...
<template>
    <div>
        <main>
            <div class="lobby">
                <div class="players">
                    <div class="icon" v-for="(position, index) in imglist" :key="index">
                        <div class="team_blue"> {{ Player_store.get_player_info()[`blue_player_${index + 1}`].nickname}}
                            <button v-if="!Player_store.get_player_info()[`blue_player_${index + 1}`].nickname" @click="enter('blue', index)">참가</button>
                        </div>
                        <img :src ="`${position}`">
                        <div class="team_red">{{ Player_store.get_player_info()[`red_player_${index + 1}`].nickname}}
                            <button v-if="!Player_store.get_player_info()[`red_player_${index + 1}`].nickname" @click="enter('red', index)">참가</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="btn-container">
                <button class="btn" @click="start_match()" v-if="Player_store.get_player_info().blue_player_1.socket_id == Socket_Store.get_socket_id()">경기 시작</button>
            </div>
        </main>
        ...
        <footer>
            <div class="bench">
                ...
                <div class="bench_player" v-for="(bench_player, index) in Player_store.get_player_info().bench">{{bench_player.nickname}}</div>
            </div>
        </footer>
    </div>
</template>
